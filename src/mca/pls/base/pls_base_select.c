...
OBJ_CLASS_INSTANCE(orte_pls_base_selected_t,
    ...
    bool *have_threads)
{
    ...
    for (item = ompi_list_get_first(&orte_pls_base.pls_components);
         ...
         * selected
         */
        module = component->pls_init(&multi, &hidden);
                                                                                                               
        /* If we got a non-NULL module back, then the component wants to
         * be selected.
         */
        if (NULL != module) {
            orte_pls_base_selected_t* selected = OBJ_NEW(orte_pls_base_selected_t);
            selected->module = module;
            selected->component = component;
            ompi_list_append(&orte_pls_base.pls_selected, &selected->super);
        }
    }
    return (ompi_list_get_size(&orte_pls_base.pls_selected) > 0) ? ORTE_SUCCESS : ORTE_ERROR;
}
