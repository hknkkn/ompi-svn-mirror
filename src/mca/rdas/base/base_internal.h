...
#ifndef MCA_RDAS_BASE_INTERNAL_H_
#define MCA_RDAS_BASE_INTERNAL_H_
 ...
 */
struct mca_rdas_base_hostfile_node_t {
...
typedef struct mca_rdas_base_hostfile_node_t mca_rdas_base_hostfile_node_t;
/** create the required instance information */
OBJ_CLASS_DECLARATION(mca_rdas_base_hostfile_node_t);


/**
 * extra data for the \c ompi_rte_node_allocation_t structures when
 * using the \c mca_rdas_base_* functions.
 */
...
typedef struct mca_rdas_base_hostfile_data_t mca_rdas_base_hostfile_data_t;
/** create the required instance information */
OMPI_DECLSPEC OBJ_CLASS_DECLARATION(mca_rdas_base_hostfile_data_t);
...
extern "C" {
     ...
     * unit tests or mca_rdas_base_open.  In other words, you probably
     * don't want to call this function.
    */ 
    OMPI_DECLSPEC void mca_rdas_base_setup(void);
     ...
     *         mca_rdas_base_hostfile_node_t information.
     */
    OMPI_DECLSPEC ompi_list_t *mca_rdas_base_parse_hostfile(const char* filename);
     ...
     *                 mca_rdas_base_hostfile_node_t instances.
     *
     * \note If the same key is used with different values, the hosts
     * are considered different.
     */
    OMPI_DECLSPEC int mca_rdas_base_collapse_resources(ompi_list_t *hostlist);
     ...
     *                 mca_rdas_base_hostfile_node_t instances.
     */
    OMPI_DECLSPEC int mca_rdas_base_map_resources(int nodes,
                                   int procs,
                                   ompi_list_t *hostlist);
     ...
     * mca_rdas_base_hostfile_node_t instances and wrap it in an
     * ompi_node_allocation_t list.
     */
    OMPI_DECLSPEC ompi_list_t *mca_rdas_base_create_node_allocation(ompi_list_t *hostlist);

    /**
     * free a list of mca_rdas_base_hostfile_node_t instances
     */
    OMPI_DECLSPEC void mca_rdas_base_deallocate(ompi_list_t *hostlist);
...
/**
 * Mutex wrapping the \code mca_rdas_base_parse_hostfile function.
 */
OMPI_DECLSPEC extern ompi_mutex_t mca_rdas_base_parse_mutex;
