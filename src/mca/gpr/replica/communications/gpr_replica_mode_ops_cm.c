...
int orte_gpr_replica_recv_triggers_active_cmd(orte_buffer_t *cmd, orte_buffer_t *answer)
{
    orte_gpr_cmd_flag_t command=ORTE_GPR_TRIGGERS_ACTIVE_CMD;
    ...
    n = 1;
    if (ORTE_SUCCESS != (rc = orte_dps.unpack(cmd, &jobid, &n, ORTE_JOBID))) {
        ORTE_ERROR_LOG(rc);
        ret = rc;
        goto RETURN_ERROR;
    }
    ...
    ret = orte_gpr_replica_triggers_active_fn(jobid);
...
int orte_gpr_replica_recv_triggers_inactive_cmd(orte_buffer_t *cmd, orte_buffer_t *answer)
{
    orte_gpr_cmd_flag_t command=ORTE_GPR_TRIGGERS_INACTIVE_CMD;
    orte_jobid_t jobid=0;
    size_t n;
    int rc, ret;
    ...
    OMPI_THREAD_LOCK(&orte_gpr_replica_globals.mutex);
    ret = orte_gpr_replica_triggers_inactive_fn(jobid);
    OMPI_THREAD_UNLOCK(&orte_gpr_replica_globals.mutex);
