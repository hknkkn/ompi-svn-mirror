...
int orte_gpr_proxy_test_internals(int level, ompi_list_t **test_results)
{
    ...
    *test_results = OBJ_NEW(ompi_list_t);
    ...
    if (orte_gpr_proxy_globals.compound_cmd_mode) {
	   if (ORTE_SUCCESS != (rc = orte_gpr_base_pack_test_internals(orte_gpr_proxy_globals.compound_cmd, level))) {
           ORTE_ERROR_LOG(rc);
       }
       return rc;
    }
    ...
    if (ORTE_SUCCESS != (rc = orte_gpr_base_unpack_test_internals(answer, &ret, test_results))) {
        ORTE_ERROR_LOG(rc);
        OBJ_RELEASE(answer);
    }
