...
int main(int argc, char *argv[])
{
    ...
    if (OMPI_SUCCESS != (ret = mca_base_cmd_line_setup(cmd_line))) {
        ompi_show_help("help-orted.txt", "orted:init-failure", true, 
    ...
    /* Open the error manager so we can report errors */
    if (ORTE_SUCCESS != (ret = orte_errmgr_base_open())) {
        printf("orted: can't open errmgr base\n");
        return ret;
    }
    
   /* check for debug flag */
    ret = mca_base_param_register_int("daemon","debug", NULL, 1);
    mca_base_param_lookup_int(ret, &orted_globals.debug);
    
    /* check to see if I'm a bootproxy */
    ret = mca_base_param_register_int("orte","bootproxy", NULL, 1);
    mca_base_param_lookup_int(ret, &orted_globals.bootproxy);
    if (orted_globals.bootproxy) { /* go fork/exec somebody and die */
...
#ifndef WIN32
    if (!orted_globals.bootproxy || !orted_globals.probe) {
    ...
    OBJ_CONSTRUCT(&orte_daemon_mutex, ompi_mutex_t);
    OBJ_CONSTRUCT(&orte_daemon_condition, ompi_condition_t);

    /* Join the run-time environment */
    allow_multi_user_threads = true;
    have_hidden_threads = false;
    if (OMPI_SUCCESS != (ret = orte_init(cmd_line, &allow_multi_user_threads,
					     &have_hidden_threads))) {
        ompi_show_help("help-orted.txt", "orted:init-failure", true,
                       "orte_init()", ret);
        return ret;
	...
	if (!orte_process_info.seed) { /* if I'm not the seed... */
	    ...
	    orte_registry.begin_compound_cmd();
	}
